...
27. **Password Policy & Complexity Synchronization**: User passwords (`CreateUserRequest.password` and `ChangePasswordRequest.newPassword`) must enforce minimum 8 characters with complexity requiring at least one uppercase letter, one lowercase letter, and one digit (`@Pattern(regexp = "^(?=.*[a-z])(?=.*[A-Z])(?=.*\\d).+$")`). Frontend password input forms in `UserListPage.tsx` and `MyAccountModal.tsx` must mirror the identical complexity and length rules client-side — any future changes to password policy must be updated synchronously in both backend DTOs and frontend forms.
