...
| Thay đổi |
|---|
| Bỏ hẳn **Test Plan** (không còn nhắc tới ở roadmap) |
| **Milestone** quay lại MVP nhưng ở dạng tối giản: chỉ là nhãn (tên + hạn chót), gắn tùy chọn vào Test Run |
| Ngôn ngữ mặc định **English**, tiếng Việt (i18n) để Phase 2 |
| Đổi tên **Folder → Section**, hỗ trợ **Subsection** (cây phân cấp, không giới hạn độ sâu) |
| **Tester được phép Import Excel** (trước đây chỉ Export) |
| Test Case thêm trường **Test Data** |
| Type thêm loại **Usability** |
| Đơn giản hóa trạng thái Test Case còn 3 mức: **Draft / Review / Ready** (bỏ Rejected, Deprecated riêng) |
| Hệ thống chỉ có **1 Leader duy nhất** (seed sẵn, không tạo thêm qua UI) |
| Vấn đề Deploy production & Real-time: **để quyết định sau**, MVP dùng Docker Compose |
...
- FR-17: **Cả Leader và Tester** import Test Case từ Excel theo template chuẩn — case import vào luôn ở trạng thái Draft.
...
- FR-21: Leader tạo Milestone (name, due date) — độc lập, không thuộc Project cụ thể hoặc thuộc 1 Project (xem giả định mục 12).
...
### 8.1 Template Import — Sheet `TestCases`

| Cột | Bắt buộc | Kiểu dữ liệu | Ghi chú |
|---|:---:|---|---|
| Section Path | ✅ | Text | Phân cấp bằng `/`, VD: `Login/Negative Case`. Tự tạo Section/Subsection nếu chưa tồn tại |
| Title | ✅ | Text | |
| Pre-condition | ✅ | Text | |
| Steps | ✅ | Text (mỗi dòng 1 bước) | |
| Expected Result | ✅ | Text | |
| Test Data | ❌ | Text | |
| Priority | ✅ | Enum | `Low`/`Medium`/`High`/`Critical` |
| Type | ✅ | Enum | `Functional`/`Regression`/`Smoke`/`Performance`/`Security`/`Usability`/`Other` |
| Automation Status | ❌ | Enum | `Manual`/`Automated`/`To Automate` — mặc định `Manual` |

> Import vào luôn ở trạng thái **Draft** — áp dụng cho cả Leader và Tester.

### 8.2 Quy trình Import (2 bước)
1. **Validate/Preview**: kiểm tra bắt buộc + enum hợp lệ, trả bảng preview + lỗi theo dòng — chưa ghi DB.
2. **Confirm Import**: ghi DB, toàn bộ ở trạng thái Draft.

### 8.3 Export
- **Export Test Case**: đúng format Sheet `TestCases` + thêm cột "Status".
...
| POST | `/api/cases/import/validate`, `/api/cases/import/confirm` | Import Excel (Leader & Tester) |
